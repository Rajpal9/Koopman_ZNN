...
class KoopmanNet(nn.Module):
  ...
  def construct_encoder_(self):
    ...
    delays = self.net_params["num_delays"]
    ...
    if hidden_depth > 0:
      # input layer
      self.encoder_fc_in = nn.Linear(input_dim * (delays+1), hidden_width)
...
class KoopmanNetCtrl(KoopmanNet):
    ...
    def forward(self, data):
        ...
        x = data[:, :n*(d+1)]
        u = data[:, n*(d+1):n*(d+1)+m]
        x_prime = data[:, n*(d+1)+m:]
    ...
    def process(self, data_x, data_u = None, downsample_rate = 1, train_mode =True):
        n = self.net_params['state_dim']
        m = self.net_params['ctrl_dim']
        d = self.net_params["num_delays"]+1
        ...
        order = 'F'
        num_snaps = data_x.shape[1]
        n_data_pts = n_traj * (data_x.shape[1]-d)
        x_flat = np.empty((n*d, n_data_pts))
        x_prime_flat = np.empty((n*d, n_data_pts))

        for i in range(d):
            x_flat[n*i:(i+1)*n,:] = data_scaled_x[:, i:-d+i, :].T.reshape((n, n_data_pts), order=order)
            x_prime_flat[n*i:(i+1)*n,:] = data_scaled_x[:, i+1:num_snaps-d+i+1, :].T.reshape((n, n_data_pts), order=order)

        x_flat_recent = x_flat[-n:,:]
        x_prime_flat_recent = x_prime_flat[-n:,:]
        u = data_scaled_u[:,d-1:,:]
        # x_prime = data_scaled_x[:,d:,:]
        ...
        #y = x_prime_flat.T
        y = np.concatenate((x_flat_recent.T, x_prime_flat_recent.T-x_flat_recent.T), axis=1)

        if train_mode:
            self.loss_scaler_x = torch.Tensor(np.std(x_prime_flat_recent[:n, :].T - x_flat_recent[:n, :].T, axis=0))
            self.loss_scaler_z = np.std(x_prime_flat_recent.T - x_flat_recent.T)
