...
def path_pars(t,t_end,c,tilt,rd_init, shape):
    ...
    #cardioid path
        n = t_end/10
        rd = np.array([2*c*np.sin(0.2*np.pi*t/n)-c*np.sin(0.4*np.pi*t/n)+rd_init[0],
        ...
        rd_dot = np.array([2*c*(0.2*np.pi/n)*np.cos(0.2*np.pi*t/n)- c*(0.4*np.pi/n)*np.cos(0.4*np.pi*t/n),
        ...
        rd_ddot =np.array([-2*c*((0.2*np.pi/n)**2)*np.sin(0.2*np.pi*t/n) + c*((0.4*np.pi/n)**2)*np.sin(0.4*np.pi*t/n),
        ...
        rd = np.array([-3*c*np.sin(0.2*np.pi*t/n) + 2*c*np.sin(0.4*np.pi*t/n)+rd_init[0],
        ...
        rd_dot = np.array([-3*c*(0.2*np.pi/n)*np.cos(0.2*np.pi*t/n) + 2*c*(0.4*np.pi/n)*np.cos(0.4*np.pi*t/n),
        ...
        rd_ddot =np.array([-3*c*((0.2*np.pi)**2)*np.sin(0.2*np.pi*t)-2*c*((0.4*np.pi)**2)*np.sin(0.4*np.pi*t),
        ...
        rd = np.array([2*c*np.sin(0.2*np.pi*t/n) - c*np.sin(0.8*np.pi*t/n)+rd_init[0],
        ...
        rd_dot = np.array([2*c*(0.2*np.pi/n)*np.cos(0.2*np.pi*t/n) - c*(0.8*np.pi/n)*np.cos(0.8*np.pi*t/n),
        ...
        rd_ddot =np.array([-2*c*((0.2*np.pi/n)**2)*np.sin(0.2*np.pi*t/n) + c*((0.8*np.pi/n)**2)*np.sin(0.8*np.pi*t/n),
        ...
        rd = np.array([c*np.sin(0.2*np.pi*t/n) - c*np.sin(0.6*np.pi*t/n)+rd_init[0],
        ...
        rd_dot = np.array([c*(0.2*np.pi/n)*np.cos(0.2*np.pi*t/n) - c*(0.6*np.pi/n)*np.cos(0.6*np.pi*t/n),
        ...
        rd_ddot =np.array([-c*((0.2*np.pi/n)**2)*np.sin(0.2*np.pi*t/n) + c*((0.6*np.pi/n)**2)*np.sin(0.6*np.pi*t/n),
        ...
        rd_dot = np.array([ c*(0.2*np.pi/n)*np.cos(0.2*np.pi*t/n),
                           -c*(0.2*np.pi/n)*np.sin(0.2*np.pi*t/n),
                            c*(0.2*np.pi/n)*np.cos(0.2*np.pi*t/n)*np.sin(tilt)])
        
        
        rd_ddot =np.array([-c*((0.2*np.pi/n)**2)*np.sin(0.2*np.pi*t/n),
    ...
    #star shape
        ...
        rd = np.array([c*np.sin(0.6*np.pi*t/n)-2*c*np.sin(0.4*np.pi*t/n)+rd_init[0],
        ...
        rd_dot = np.array([c*(0.6*np.pi/n)*np.cos(0.6*np.pi*t/n) -2*c*(0.4*np.pi/n)*np.cos(0.4*np.pi*t/n),
        ...
        rd_ddot =np.array([-c*((0.6*np.pi/n)**2)*np.sin(0.6*np.pi*t/n)+2*c*((0.4*np.pi/n)**2)*np.sin(0.4*np.pi*t/n),
...
def path_pars_2D(t,t_end,c,tilt,rd_init, shape):
    ## Inputs
    # t - current time, 
    # t_end - time taken to complete the path,
    # c - size of the shape, 
